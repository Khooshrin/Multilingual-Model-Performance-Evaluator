...
def find_suggestions(
    ...
    for tgt, score in zip(langs_tgts, TgtPerfs(equal_aug_sizes)):
        print(tgt + "  :  " + score)
...
def get_user_config_perfs(model, featurizer, langs_list, targets, data_sizes):
    ...
    print(tgt_perfs)
    with open("data/precomputed_features.json") as fin:
        precomputed = json.load(fin)
    precomputed_type_overlaps = precomputed['type_overlap'][args.model_name]
    #type_overlaps = [[precomputed_type_overlaps[pivot][lang] for lang in langs_tgts] for pivot in langs_pvts]

    input=args.data_sizes
    eachinput=[everyinput for everyinput in input.split(",")]
    eachinputlang=[]
    for langonly in eachinput:
        langsizelist = langonly.split(":")
        eachinputlang.append(langsizelist[0])

    recall=[]
    for lang,tgt in zip(langs_tgts,tgt_perfs):
        falspos=0.0
        for pivot in eachinputlang:
            falspos = falspos + precomputed_type_overlaps[lang][pivot]
        falspos = falspos / len(eachinputlang)
        falspos = tgt/(tgt+falspos)
        recall.append(falspos)

    newscore=[]
    for curscore,currecall in zip(tgt_perfs,recall): 
        temp = curscore*currecall+(currecall/1.49)
    ...
    return {
        ...
        #"type_overlaps_size" : len(type_overlaps),
        "tgt_perfs" : tgt_perfs,
        "tgt_lang" : langs_tgts,
        "pvt_lang" : eachinputlang,
        "recall" : recall,
        "new_score" : newscore,
    }
...
def litmus_main(args):
    ...
    if args.load_state:
        ...
        #get_user_config_perfs(featurizer,langs_list,langs_target,)
    else:
        # Prepare data
